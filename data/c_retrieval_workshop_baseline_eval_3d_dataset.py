...
class RetrievalWorkshopBaselineEval3DDataset(BaseDataset):
    def __init__(self, opt):
        ...
        #self.input_3d_path = '/home/dh/zdd/data/validation_retrieval/render5'#os.path.join(opt.dataroot, 'notexture_pool_data')
        self.input_2d_path = os.path.join(opt.dataroot, 'input_data')
        # 3 视图
        #self.input_3d_path ='/home/dh/zdd/data/test/render5_black/' #os.path.join(opt.dataroot, 'notexture_pool_data')
        # 5 视图
        #self.input_3d_path ='/home/dh/zdd/dataset/test_data/render5_black' #os.path.join(opt.dataroot, 'notexture_pool_data')
    ...
    def _load_3d_image(self, shape_id, image_name):
        ...
        # 5 views
        img_file = [os.path.join(self.input_3d_path, '%07d.png' % (int(shape_id)*5+i)) for i in range(5)]
        # 12 views
        #angle = torch.arange(0,360,30)
        ##angle[2] = 75
        #img_file = [os.path.join(self.input_3d_path, shape_id, 'image_%03d.png' % (int(angle[i]))) for i in range(12)]
        trans_img = []
        for f in img_file:
